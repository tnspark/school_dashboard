...
if df is not None:  # Proceed only if data loading was successful
    ...
    def _read_csv_with_fallback(file_obj):
        """
        Attempt to read a CSV from an uploaded file-like object.
        Tries UTF-8 first, then falls back to Latin-1 encoding if needed.

        Args:
            file_obj: A file-like object from Streamlit's uploader.

        Returns:
            pd.DataFrame or None: Parsed DataFrame on success, None on failure.
        """
        try:
    ...
    def _concat_with_union(dfs):
        """
        Concatenate multiple DataFrames while aligning columns by union.
        Missing columns are introduced as NaN so shapes match.

        Args:
            dfs (List[pd.DataFrame]): List of DataFrames to merge.

        Returns:
            pd.DataFrame or None: Concatenated DataFrame, or None if input is empty.
        """
        if not dfs:
            return None
    ...
    with tab_syllabus:
        ...
        if 'class_id' in filtered_df.columns and 'completion_percentage' in filtered_df.columns:
            ...
            # Note: AG Grid 'sideBar' requires enterprise modules. Disabling to avoid error #200.
    ...
    with tab_more:
        ...
        with col2:
            ...
            # If you correct it in the source to 'attendance_status', update the check below accordingly.
        ...
        if 'activity_date' in filtered_df.columns and 'completion_percentage' in filtered_df.columns:
            ...
            def compute_completion_trend(df):
                """
                Compute daily average completion percentage and a 7-day rolling mean.

                Args:
                    df (pd.DataFrame): Filtered dataset containing 'activity_date' and 'completion_percentage'.

                Returns:
                    pd.DataFrame: DataFrame with columns ['activity_date', 'avg_completion', 'rolling_7d'].
                """
                ts = (
        ...
        if 'school_name' in filtered_df.columns and 'activity_date' in filtered_df.columns:
            ...
            def compute_alerts(df, days_without_activity=7):
                """
                Identify schools with stale activity and classes with declining completion.

                Args:
                    df (pd.DataFrame): Filtered dataset.
                    days_without_activity (int): Threshold in days to flag inactivity.

                Returns:
                    Tuple[pd.DataFrame, pd.DataFrame]:
                        - stale schools with last activity date
                        - top classes with negative delta in completion week-over-week
                """
        ...
        if 'school_name' in filtered_df.columns and 'activity_date' in filtered_df.columns and 'completion_percentage' in filtered_df.columns:
            ...
            def compute_heatmap(df):
                """
                Construct a school-by-month matrix of average completion percentages.

                Args:
                    df (pd.DataFrame): Filtered dataset.

                Returns:
                    pd.DataFrame: Pivoted matrix suitable for heatmap visualization.
                """
                df2 = df.copy()
